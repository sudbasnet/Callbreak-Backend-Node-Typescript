...
class Deck {
    ...
    getFullDeck() {
        let deck: Card[] = [];
        Deck.suites.forEach(suit => Deck.numberCards.forEach(numberCard => deck.push(new Card(suit, numberCard))));
        Deck.suites.forEach(suit => Deck.faceCards.forEach(faceCard => deck.push(new Card(suit, faceCard))));
        return deck;
    }

    getRandomLocation(len: number) {
        return Math.floor(Math.random() * Math.floor(len));
    }

    dealCards(numberOfCards: number, numberOfPlayers: number) {
